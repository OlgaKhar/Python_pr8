...
def copy_data(file_name, out_file):
    try:
        copyfile(file_name, out_file)
        print(f"Данные из файла {file_name} успешно скопированы в {out_file}")
    except Exception as e:
        print(f"Произошла ошибка при копировании данных: {e}")


def copy_strok(file_name):
    row_number = int(input('Введите номер строки для копирования: '))
    res = read_file(out_file)
    print(f"Данные из файла {file_name} успешно скопированы в {out_file}")

    if row_number <= len(res):
        res.append(res[row_number-1])
        standart_write(out_file, res)
    else:
        print("Введен неверный номер строки")
...
print("Пожалуйста, выберите одну из команд для выполнения действия:")
print("'q' - для выхода из программы")
print("'w' - для записи новой записи в файл")
print("'r' - для чтения данных из файла")
print("'d' - для удаления строки из файла")
print("'c' - для копирования данных из файла в другой файл")
print("'s' - для копирования строки из файла в другой файл")
    ...
    while True:
        ...
        if command == 'q':
            ...
            if not exists(file_name):
                ...
                continue
            copy_data(file_name, out_file)


        elif command == 's':
            if not exists(file_name):
                create_file(file_name)
            copy_strok(file_name)
